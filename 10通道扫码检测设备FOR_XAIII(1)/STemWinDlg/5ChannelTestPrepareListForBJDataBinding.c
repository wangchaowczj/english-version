		...
		case WM_TIMER:
			switch(WM_GetTimerId(pMsg->Data.v))
			{
				case GUI_TIMER_1MS_ID:				
                    ...
                    if((UserParam.DetOutsideStartBoxIndex%5) != 0)
                    {
                        ShowCustomMessageBox("管码最后1位必需是0或5", OpData.DlgTitle, MB_OK);
                        ShowDetOutsideForBJDlg(1);
                        return;
                    }

                    if(((UserParam.DetOutsideStartBoxIndex+5)%100) > UserParam.InBoxNumber)
                    {
...
void Show5ChannelTestPrepareListForBJDataBindingDlg(void)
{
