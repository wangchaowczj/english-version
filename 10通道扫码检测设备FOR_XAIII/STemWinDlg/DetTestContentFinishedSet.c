...
static const GUI_WIDGET_CREATE_INFO Res[] = {
  ...
  { TEXT_CreateIndirect,   "芯片类型:",       GUI_ID_TEXT0,     35, 40, 108,  25,  0,          0 },
  { TEXT_CreateIndirect,   "延期检测:",       GUI_ID_TEXT1,     35, 70, 108,  25,  0,          0 },
		...
		case WM_INIT_DIALOG://初始化界面
			...
			switch(UserParam.DetType)
			{
				case DET_TYPE_XA_BD1:
					MULTIEDIT_SetText(WM_GetDialogItem(hWin, GUI_ID_EDIT0), "XA-BD1");					
					break;
				case DET_TYPE_XA_BD2:
					MULTIEDIT_SetText(WM_GetDialogItem(hWin, GUI_ID_EDIT0), "XA-BD2");					
					break;					
				case DET_TYPE_XA_PD1:
					MULTIEDIT_SetText(WM_GetDialogItem(hWin, GUI_ID_EDIT0), "XA-PD1");					
					break;
				case DET_TYPE_XA_PD2:
					MULTIEDIT_SetText(WM_GetDialogItem(hWin, GUI_ID_EDIT0), "XA-PD2");
					break;
				case DET_TYPE_XA_PD3:
					MULTIEDIT_SetText(WM_GetDialogItem(hWin, GUI_ID_EDIT0), "XA-PD3");
					break;					
			}
			if(UserParam.FinishedDelayItem)
			{
		...
		case WM_KEY:		
			switch (((WM_KEY_INFO*)(pMsg->Data.p))->Key) 
			{
				case GUI_CUSTOM_KEY_OK:															
					...
					if((UserParam.DetType != DetType) || (UserParam.FinishedDelayItem != FinishedDelayItem) || (UserParam.DetDelay != Delay))
					{
...
//						ClearDetInfoQueue(&DetInfoQueue);
						...
						UserParam.FinishedDelayItem = FinishedDelayItem;
						UTF8StringToGBK("成品检延期检测", (char*)OpData.GuiTemp);						
						if(WriteKeyU32Value(CONFIG_PARAM_FILE_COMPLETE_PATH, OpData.GuiTemp, UserParam.FinishedDelayItem) == ERROR)
						{
							ShowCustomMessageBox("保存失败!", OpData.DlgTitle, MB_OK);
							return;
						}
...
void ShowDetTestContentFinishedSetDlg(void)
{
