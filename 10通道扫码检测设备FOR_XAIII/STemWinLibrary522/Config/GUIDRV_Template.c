...
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex) {
    ...
    GUI_USE_PARA(PixelIndex);
    {
	  ...
	  //LcdSetPixel(x, y, PixelIndex);
		//直接在这用命令方式，免去函数调用时的出入栈，速度更快
...
static unsigned int _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  unsigned int PixelIndex,PixelIndex1;
    //
    // Convert logical into physical coordinates (Dep. on LCDConf.h)
    //
    #if (LCD_MIRROR_X == 1) || (LCD_MIRROR_Y == 1) || (LCD_SWAP_XY == 1)
...
//		LcdFsmcLock();
		do
		{
			...
			LCD_DataRead();//舍弃
			LCD_DataRead();//舍弃
			PixelIndex = LCD_DataRead();
			PixelIndex1 = LCD_DataRead();			
		}
		while(PixelIndex != PixelIndex1);
...
//	LcdFsmcLock();
	...
	LcdBTESetTarBlock(x0, y0, y1-y0+1, x1-x0+1, 0);	/* 设置BTE位置和宽度高度 */
	LcdBTESetOperateCode(0x0C);	        			/* 设定BTE 操作码和光栅运算码  REG[51h] Bit[3:0] = 0Ch */
	LcdSetFrontColor(PixelIndex);			/* 设置BTE前景色 */
	LcdBTEStart();				        			/* 开启BTE 功能 */
	LcdBTEWait();					        			/* 等待操作结束 */
 
//	LcdFsmcUnlock();
//	LcdSetFrontColor(PixelIndex);
//	
//	LcdDrawRect(x0,y0,x1,y1,1);
  }
}
