...
#define APP_PUBLIC_TASK_MAX   6 //最多允许同时使用公共堆栈的任务数
	...
	switch(prio)
	{
		...
		case APP_USER_GUI_TASK_PRIO:
			Result = OSTaskCreateExt(AppUserGUITask, (void*)p_arg,
						&PrivateStk.UserGUI[APP_USER_GUI_TASK_STK_SIZE-1], prio,
						prio,&PrivateStk.UserGUI[0], APP_USER_GUI_TASK_STK_SIZE,
						(void *)0,OS_TASK_OPT_STK_CHK | OS_TASK_OPT_STK_CLR);	
			break;	
		case APP_KEYSCAN_TASK_PRIO:
			Result = OSTaskCreateExt(AppKeyScanTask, (void*)p_arg,
						&PrivateStk.KeyScan[APP_KEYSCAN_TASK_STK_SIZE-1], prio,
						prio,&PrivateStk.KeyScan[0], APP_KEYSCAN_TASK_STK_SIZE,
						(void *)0,OS_TASK_OPT_STK_CHK | OS_TASK_OPT_STK_CLR);	
			break;
			
		case APP_DET_DETECTOR_TASK_PRIO:
			PublicTask = DetDetectorTask;	
			break;			
		case APP_BACKGROUND_TASK_PRIO:
			PublicTask = AppBackgroundTask;	
			break;
		case APP_USB_TASK_PRIO:
			PublicTask = AppUsbTask;		
			break;
		case APP_CH374_TASK_PRIO:
			PublicTask = AppCH374Task;
			break;
		case APP_SOUND_TASK_PRIO:
			PublicTask = AppSoundTask;
			break;
		case APP_FACTORY_API_TASK_PRIO:
			PublicTask = AppFactoryApiTask;		
			break;		
		default:
			break;
	}
