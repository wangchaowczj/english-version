...
* @retval None
*/
void USBH_USR_DeviceAttached(void)				//应该是设备已连接到主机，即设备已插入
{  
//  LCD_UsrLog ((void*)MSG_DEV_ATTACHED);  		"> Device Attached\n"
	Usb_Trans.USB_State = 2;
}
...
* @retval None
*/
void USBH_USR_UnrecoveredError (void)		//无法恢复的错误状态？？？，被usbh_core.c的377行调用
{
//  LCD_ErrLog((void*)MSG_UNREC_ERROR);		"> UNRECOVERED ERROR STATE\n"
}
...
* @retval None
*/
void USBH_USR_DeviceDisconnected (void)			//设备已拔出
{
  
	Usb_Trans.USB_State = 0xFE;
...
* @retval None
*/
void USBH_USR_ResetDevice(void)			//复位设备
{
...
* @retval None
*/
void USBH_USR_DeviceSpeedDetected(uint8_t DeviceSpeed)		//USB从机速度模式：低速，全速，高速，咱们用的都是全速模式，即FS
{
...
* @retval None
*/
void USBH_USR_Device_DescAvailable(void *DeviceDesc)		//从机信息，如VID和PID信息
{
//  uint8_t temp[50];    
...
* @retval None
*/
void USBH_USR_Configuration_DescAvailable(USBH_CfgDesc_TypeDef * cfgDesc,			//显示设备连接方式，如HID或MSC。咱们用的是HID模式
                                          USBH_InterfaceDesc_TypeDef *itfDesc,		//但使用的是自定义HID模式。
...
* @retval None
*/
void USBH_USR_Manufacturer_String(void *ManufacturerString)				//从机设备信息
{
//  char temp[100];
...
* @retval None
*/
void USBH_USR_Product_String(void *ProductString)						//从机产品信息
{
//  char temp[100];
...
* @retval None
*/
void USBH_USR_SerialNum_String(void *SerialNumString)				//序列号信息
{
//  uint8_t temp[100];
...
* @retval None
*/
void USBH_USR_EnumerationDone(void)						//从机枚举成功
{
//	USB_Host_State = 3;
...
* @retval None
*/
void USBH_USR_DeviceNotSupported(void)				//设备类型不支持
{
...
*/
USBH_USR_Status USBH_USR_UserInput(void)			//用户启动。
													//该函数被调用之前，已经枚举成功。
													//该函数可用来启动数据传输。
													//该函数返回USBH_USR_RESP_OK则表示用户启动数据传输
{
...
* @retval None
*/
void USBH_USR_OverCurrentDetected (void)			//USB过流。这一项咱们用不到。
{
...
* @retval None
*/
void  USR_HID_CUSTOM_ProcessData (uint8_t data)
{
