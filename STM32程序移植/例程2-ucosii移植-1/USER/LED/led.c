...
void LED_Init(void)
{
 ...
 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15;	    		 //LED1-->PA.15 端口配置, 推挽输出
 GPIO_Init(GPIOA, &GPIO_InitStructure);	  				 //推挽输出 ，IO口速度为50MHz
 GPIO_SetBits(GPIOA,GPIO_Pin_15); 						 //PA.15 输出高 
	
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;			//OUT->PA0
	GPIO_Init(GPIOA,&GPIO_InitStructure);						//初始化端口GPIOA
	GPIO_ResetBits(GPIOA,GPIO_Pin_0);								//输出低

 //GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7;	    		 //LED1-->PA.15 端口配置, 推挽输出
 //GPIO_Init(GPIOE, &GPIO_InitStructure);	  				 //推挽输出 ，IO口速度为50MHz
 //GPIO_SetBits(GPIOA,GPIO_Pin_15); 						 //PA.51 输出高 
}
