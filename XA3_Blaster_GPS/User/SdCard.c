...
void SdCardSpiConfig(void)
{
	...
	RCC_APB1PeriphClockCmd(SDCARD_SPI_RCC, ENABLE);
	
	GPIO_PinAFConfig(SDCARD_SPI_SCLK_PORT, GPIO_PinSource10, GPIO_AF_SPI3);
	GPIO_PinAFConfig(SDCARD_SPI_MISO_PORT, GPIO_PinSource11, GPIO_AF_SPI3);
	GPIO_PinAFConfig(SDCARD_SPI_MOSI_PORT, GPIO_PinSource12, GPIO_AF_SPI3);
	...
	SPI_InitStruct.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_8;        // 在72Mhz的主频下，APB1为36Mhz(2分频)，波特率8分频后约4.5MHz
...
ErrorStatus SdCardWriteBlock(const uint8_t *buff, uint32_t sector, uint32_t count)
{
    ...
    else
    {
		...
		{
			SdCardSendCmd(SDCARD_ACMD23, SectorNumber, tmp, 1);
		}			
        /* 发送CMD25 */
        ...
        {// CMD25指令执行正常
            do
            {
                rtn = SdCardSendDataBlock(buff+Index, SDCARD_CMD25_TOKEN);
                if(rtn==ERROR)
                {
                    break;
                }
...
//		for(SectorNumber = 0;SectorNumber<count;SectorNumber++)
//		{
//			if(ERROR == SdCardReadBlock(VerifyBuffer,sector+SectorNumber,1))
//			{
//				if(ERROR == SdCardReadBlock(VerifyBuffer,sector,1))
//				{
//					return ERROR;
//				}			
//			}
//			if(memcmp(VerifyBuffer, buff+Index, 512) != 0)
//			{
//				return ERROR;
//			}
//			Index += 512;
