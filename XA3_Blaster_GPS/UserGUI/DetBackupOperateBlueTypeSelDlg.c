...
static void Callback(WM_MESSAGE * pMsg) 
{
	...
	switch (pMsg->MsgId) 
	{
		case WM_PAINT://绘制界面
			...
			GUI_DispStringHCenterAt("1.Bluetooth Master Mode", 120, 90);//1.蓝牙主机模式
			GUI_DispStringHCenterAt("2.Bluetooth Slave Mode ",120, 115);	//2.蓝牙从机模式
		...
		case WM_KEY:
			...
			switch (((WM_KEY_INFO*)(pMsg->Data.p))->Key) 
			{
				case GUI_KEY_LEFT:
					...
					break;					
				case GUI_KEY_1://主机模式
					ShowBlueHostStatusDlg(3);
					break;
				case GUI_KEY_2://从机模式
					Op_Data.BlueSlaveStatus = DET_INFO_INSERT;
...
void ShowDetBackupOperateBlueTypeSelDlg(void)
{
	CloseMainDialog();
