...
/**
  ...
  *************************************************************************************  
  */
ErrorStatus SetWindowParam(u32 Param)
{
	if(WM_IsWindow(DlghWin.Dlg))
	{
		if (1 == WM_SetUserData(DlghWin.Dlg, &Param, 4))
		{
			return SUCCESS;
		}	
	}
...
/**
  ...
  *************************************************************************************  
  */
u32 GetWindowParam(void)
{
	u32 Param = 0;
	
	if(WM_IsWindow(DlghWin.Dlg))
	{
		WM_GetUserData(DlghWin.Dlg, &Param, 4);	
	}
...
/**
  *************************************************************************************
  * @brief  话框窗口DlghWin.Dlg是不是某个窗口的子窗口
  * @param[in]  无
  * @retval 无
  * @author ZH
  * @date   2016年12月10日
  ...
  *************************************************************************************  
  */
u8 WindowHasChild(void)
{
    if(WM_IsWindow(DlghWin.Dlg))
    {
        if(WM_HBKWIN != WM_GetParent(DlghWin.Dlg))
        {
            return 1;
        }    
    }
...
//	if(CustomMessageBoxIsShow())
//	{
//		CloseCustomMessageBoxAndParentForce();
//		while(CustomMessageBoxIsShow())
//		{
//			GUI_Delay(1);
//		}		
//	}
...
void CloseMainDialog(void)
{
    ...
    DlghWin.Dlg = 0;
    if(Op_Data.MainMenuNow)
    {//从主界面离开
        Op_Data.MainMenuNow = 0;
        CLOSE_BLUE_POWER();
    }    
}
/**
...
//	{
		//计算每行显示多少个字符
		LineCharCount = WM_GetWindowSizeX(hWin)/8-1;
...
//		LineCurrentCharCount = (MULTIEDIT_GetTextSize(hWin)-1)%LineCharCount;
		...
		GUI_DrawLine(8*LineCurrentCharCount+4, 4+16*LineCurrent, 
			8*LineCurrentCharCount+4, 16*(LineCurrent+1));		
//	}
	return LineCurrent;
}
/**
...
void ShowKeyboardCharSelectList(WM_HWIN hWin, WM_HWIN buttonHWin, WM_HWIN textHWin)
{
	u8 i, num = 0;

	memset(DlghWin.KeyboardText, 0, sizeof(DlghWin.KeyboardText));
	BUTTON_GetText(buttonHWin, (char*)DlghWin.KeyboardText, sizeof(DlghWin.KeyboardText));
	
    memset((char*)Op_Data.Temp, 0, sizeof(Op_Data.Temp));
    for(i=0; i<strlen((const char*)DlghWin.KeyboardText); i++)
	{
		num++;
		if(num == 6)
		{
			strcpy((char*)&Op_Data.Temp[strlen((const char*)Op_Data.Temp)], "\n");
		}
		sprintf((char*)&Op_Data.Temp[strlen((const char*)Op_Data.Temp)],"%d.%c ",num%10, DlghWin.KeyboardText[i]);
	}
	TEXT_SetText(textHWin, (char*)Op_Data.Temp);
}

void CustomSendMessage(WM_HWIN hWin, WM_MESSAGE * p)
{
    while(!WM_IsWindow(hWin))
    {
        OSTimeDly(1);
    }
    WM_SendMessage(hWin, p);
}
