...
/* 该程序演示处理类似硬盘的多磁盘分区，依次对各个磁盘分区做如下操作：
     将磁盘中的/CH374HFT.C文件中的前600个字符显示出来,
     如果找不到原文件CH374HFT.C, 那么该程序将显示根目录下的前254个文件名。
*/
...
*/
	...
	for ( DiskPartCount = 0; DiskPartCount < 7; DiskPartCount ++ ) {  /* 依次处理硬盘的磁盘分区,最多支持7个 */

		if ( DiskPartCount ) {  /* 非首次进入说明要启用下一个磁盘分区 */
//			if ( CH374DiskStatus > DISK_MOUNTED )  /* 非首次进入时条件总是成立 */
				CH374DiskStatus = DISK_MOUNTED;  /* 首次进入时CH374DiskStatus<DISK_MOUNTED，后次进入时则大于 */
			if ( CH374v03 == 0 ) break;  /* 是直接逻辑盘,没有磁盘分区,强制分析会返回ERR_TYPE_ERROR或者ERR_BPB_ERROR或者ERR_FAT_ERROR等错误码 */
			CH374v03 = 0x12345678 | ( DiskPartCount + 1 );  /* 指定要启用的磁盘分区 */
		}
		else { };  /* 首次进入按正常步骤 */

/* 读取原文件 */
		printf( "Open\n" );
		strcpy( mCmdParam.Open.mPathName, "/CH374HFT.C" );  /* 文件名,该文件在根目录下 */
		i = CH374FileOpen( );  /* 打开文件 */
		if ( i == ERR_MISS_DIR || i == ERR_MISS_FILE ) {  /* 没有找到文件 */
/* 列出文件 */
			if ( i == ERR_MISS_DIR ) pCodeStr = "/*";  /* 列出根目录下的文件 */
			printf( "List file %s\n", pCodeStr );
