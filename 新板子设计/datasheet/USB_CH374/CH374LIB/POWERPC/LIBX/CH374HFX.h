...
**  TC2.0@PC, Tornado 2.2 gcc @PowerPC **
****************************************
*/
/* CH374 主机文件系统接口 */
/* 支持: FAT12/FAT16/FAT32 */
/* PowerPC单片机, 查询方式或者中断方式 */
...
#if DISK_BASE_BUF_LEN
#define	DISK_BASE_BUF	((PUINT8)DISK_BASE_BUF4)
#ifdef DISK_BASE_BUF_ADDR
UINT32  DISK_BASE_BUF4[ DISK_BASE_BUF_LEN/4 ];	/* 外部RAM的磁盘数据缓冲区,缓冲区长度为一个扇区的长度,起始地址必须为4字节边界地址 */
#else
UINT32  DISK_BASE_BUF4[ DISK_BASE_BUF_LEN/4 ];	/* 外部RAM的磁盘数据缓冲区,缓冲区长度为一个扇区的长度,起始地址必须为4字节边界地址 */
#endif
#endif
#ifdef FILE_DATA_BUF_LEN
#define	FILE_DATA_BUF	((PUINT8)FILE_DATA_BUF4)
#ifdef FILE_DATA_BUF_ADDR
UINT32  FILE_DATA_BUF4[ FILE_DATA_BUF_LEN/4 ];	/* 外部RAM的文件数据缓冲区,缓冲区长度不小于一次读写的数据长度,起始地址必须为4字节边界地址 */
#else
UINT32  FILE_DATA_BUF4[ FILE_DATA_BUF_LEN/4 ];	/* 外部RAM的文件数据缓冲区,缓冲区长度不小于一次读写的数据长度,起始地址必须为4字节边界地址 */
#endif
